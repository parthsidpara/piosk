...
BINARY_FILE=$(find "$PIOSK_INSTALL_DIR" -maxdepth 1 -type f -name 'piosk-v*')
if [ -z "$BINARY_FILE" ]; then
    msg "$ERROR" "PiOSK binary not found. Package is corrupt."
    exit 1
fi
chmod +x "$BINARY_FILE"
mv "$BINARY_FILE" "$PIOSK_INSTALL_DIR/piosk" # Rename the versioned binary to 'piosk' inside /opt/piosk
msg "$SUCCESS" "Binary prepared at $PIOSK_INSTALL_DIR/piosk"
